...
const checks: Array<[string, boolean]> = [
  ...
  ['mobile uses touch guidance while desktop keyboard guidance remains unchanged', /isMobileLayout \? labels\.mobileHelp : labels\.keyboardHelp/.test(palette) && /Search or tap a command to open it\./.test(dashboard) && /Use arrow keys to navigate, Enter to open, and Escape to close\./.test(dashboard) && /ابحث أو اضغط على أمر لفتحه\./.test(dashboard)],
  ['responsive guidance follows layout and live input modality without user-agent sniffing', /isMobileLayout/.test(palette) && /keyboardModality/.test(palette) && /event\.pointerType === 'touch'/.test(palette) && !/userAgent|navigator\.platform/.test(palette)],
  ...
  ['mobile keyboard geometry follows the visual viewport and keeps results plus footer in one flex panel', /window\.visualViewport/.test(palette) && /viewport\?\.addEventListener\('resize'/.test(palette) && /style=\{isMobileLayout \? \{ height: visualViewport\.height, top: visualViewport\.offsetTop \}/.test(palette) && /min-h-0 flex-1 overflow-y-auto/.test(palette)],
  ['view-all is an accessible in-place disclosure that clears search and returns results to the top', /viewAllCommands/.test(palette) && /setQuery\(''\)/.test(palette) && /setShowAll\(true\)/.test(palette) && /scrollTo\(\{ top: 0 \}\)/.test(palette) && /aria-controls=\{resultsId\}/.test(palette) && /View all \$\{count\} commands/.test(dashboard)],
  ['suggested empty state is temporary, permission-filtered, and expands without another modal', /suggestedIds/.test(palette) && /quickActions/.test(palette) && /currentContextId/.test(palette) && /useState\(false\)/.test(palette) && (palette.match(/role="dialog"/g) || []).length === 1],
