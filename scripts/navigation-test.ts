...
const checks: Array<[string, boolean]> = [
  ...
  ['Geo Operations uses one centered workspace container with aligned summary and full-width sections', /geo-operations-content relative mx-auto flex[\s\S]*max-w-\[72rem\]/.test(dashboard) && /geo-operations-summary-grid[\s\S]*canCreateBreakRequests && "md:grid-cols-2"/.test(dashboard) && (dashboard.match(/geo-operations-full-section/g) || []).length === 2],
  ...
  ['lanyard remains a Launcher-only presentation and stays visually connected beneath Settings', /shouldMountLanyard = lanyardEnabled && isLanyardCapable && desktopNavigationMode === 'launcher'/.test(dashboard) && /showLanyardDock=\{Boolean\(launcherLanyard\)\}/.test(dashboard) && /paused=\{!isDashboardVisible\}/.test(dashboard) && /z-20/.test(nav) && /z-40/.test(dashboard)],
  ['launcher assembly has no broad interaction hitbox and keeps decorative strap inert', /pointer-events-none fixed inset-0/.test(nav) && /pointer-events-auto absolute start/.test(nav) && /pointer-events-none absolute left-1\/2 top-full/.test(nav)],
