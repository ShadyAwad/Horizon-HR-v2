...
const checks: Array<[string, boolean]> = [
  ...
  ['launcher lanyard has a transparent centred short strap in the launcher wrapper', /<div className="relative shrink-0">[\s\S]*showLanyardDock/.test(nav) && /aria-hidden="true"/.test(nav) && /left-1\/2 top-full/.test(nav) && /h-7 w-px/.test(nav) && /pointer-events-none/.test(nav)],
