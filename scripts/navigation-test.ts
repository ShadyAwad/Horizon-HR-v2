...
const checks: Array<[string, boolean]> = [
  ...
  ['mobile shortcuts use stable ids with a minimum and crowding warning', /mobileShortcuts/.test(dashboard) && /selected\.length <= 1/.test(shortcuts) && /selected\.length > 5/.test(shortcuts)],
  ...
  ['opening or searching the launcher does not record module use', /reportTutorialAction\('click', 'stanza-launcher'\)/.test(nav) && /onOpenCommandPalette\(launcherRef\.current\)/.test(nav) && !/reportTutorialAction\('click', 'stanza-launcher'\)[\s\S]{0,100}onModuleNavigate/.test(nav)],
  ...
  ['lanyard mount eligibility requires the saved preference, capable desktop viewport, and Launcher Only mode without browser or pointer heuristics', /shouldMountLanyard = lanyardEnabled && isLanyardCapable && desktopNavigationMode === 'launcher'/.test(dashboard) && /min-width: 1024px/.test(dashboard) && !/pointer: fine|hover: hover|userAgent/.test(dashboard.slice(dashboard.indexOf('const reducedMotionQuery'), dashboard.indexOf('const shouldMountLanyard'))) && /if \(!shouldMountLanyard\) \{[\s\S]*setIsLanyardIdleReady\(false\);[\s\S]*return;/.test(dashboard)],
  ['external lanyard remains mounted and visible while navigation or Settings is open', !/isLanyardSceneReady|hidden=\{/.test(dashboard) && /interactionEnabled=\{!showControlCenter && isDashboardVisible\}/.test(dashboard) && /aria-hidden=\{!interactionEnabled\}/.test(read('src/components/lanyard/StanzaDashboardLanyard.tsx'))],
  ...
  ['lanyard remains a Launcher-only presentation and stays visually connected beneath Settings', /shouldMountLanyard = lanyardEnabled && isLanyardCapable && desktopNavigationMode === 'launcher'/.test(dashboard) && /paused=\{!isDashboardVisible\}/.test(dashboard) && !/paused=\{hidden \|\| paused\}/.test(read('src/components/lanyard/StanzaDashboardLanyard.tsx')) && /z-20/.test(nav) && /z-40/.test(dashboard)],
  ...
  ['Settings has one panel title without duplicate expanded-section headings', /dash\.controlCenterTitle/.test(dashboard) && /renderControlCenterAccordion\(\s*'settings',\s*t\('dash\.settings'\)/.test(dashboard) && !/text-sm font-bold uppercase tracking-widest text-slate-800 dark:text-slate-200">\{t\('dash\.appearance'\)\}/.test(dashboard) && /'dash\.appearance': 'Appearance'/.test(language) && /'dash\.appearance': 'المظهر'/.test(language)],
  ['Settings uses a transparent portal root, a neutral pointer-blocking backdrop, and a separately themed scrollable drawer', /stanza-settings-overlay fixed inset-0 z-40/.test(dashboard) && /stanza-modal-backdrop absolute inset-0/.test(dashboard) && /stanza-settings-drawer stanza-scrollbar/.test(dashboard) && /onClick=\{\(\) => setShowControlCenter\(false\)\}/.test(dashboard) && /overflow-y-auto overscroll-contain/.test(dashboard) && /\.stanza-modal-backdrop[\s\S]*background-color: rgb\(0 0 0 \/ 0\.35\)/.test(css) && /\.stanza-settings-drawer[\s\S]*background-color: var\(--stanza-surface-panel\) !important/.test(css) && !/stanza-settings-overlay fixed inset-0 z-40 bg-/.test(dashboard)],
  ['Arabic launcher and Settings labels use concise RTL-aware terminology', ['نمط التنقل', 'زر Stanza فقط', 'شريط مختصر', 'إعادة تعيين الوحدات الأخيرة والأكثر استخدامًا', 'درجة سطوع الوضع الفاتح', 'داكن نسبيًا', 'الإعدادات'].every((copy) => language.includes(copy)) && /dash\.navigationStyle/.test(dashboard) && /dash\.launcherOnly/.test(dashboard) && /dash\.compactRail/.test(dashboard) && /isRtl && '-scale-x-100'/.test(nav)],
  ...
  ['mobile bottom navigation is content-sized and keeps the add action adjacent', /w-fit max-w-\[calc\(100vw-1\.5rem\)\]/.test(nav) && /left-1\/2 -translate-x-1\/2 md:hidden/.test(nav) && /max-w-\[calc\(100vw-7\.5rem\)\]/.test(nav)],
  ['mobile search filters the existing navigation registry instead of opening a second Command Palette', /isMobileLayout \? <label/.test(nav) && /mobileVisibleItems/.test(nav) && /setMobileSearch/.test(nav) && /onOpenCommandPalette\(launcherRef\.current\)/.test(nav)],
];
let failed = false;
