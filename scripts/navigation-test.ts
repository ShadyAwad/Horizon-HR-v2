...
const checks: Array<[string, boolean]> = [
  ...
  ['launcher lanyard has a transparent centred strap and no layout column', /showLanyardDock/.test(nav) && /aria-hidden="true"/.test(nav) && /left-1\/2 top-full/.test(nav) && /pointer-events-none/.test(nav)],
