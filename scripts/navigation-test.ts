...
const checks: Array<[string, boolean]> = [
  ...
  ['launcher-only no longer reserves a header grid slot or narrows dashboard content', /data-dashboard-context-header/.test(dashboard) && !/launcher-header-slot/.test(dashboard) && !/data-launcher-header-slot/.test(dashboard) && /data-dashboard-context-content/.test(dashboard) && /<main className="min-w-0 w-full max-w-full flex-1/.test(dashboard)],
