...
CREATE TABLE IF NOT EXISTS tenants (
    ...
    asset_label_disclosure_level VARCHAR(32) NOT NULL DEFAULT 'label_only'
...
CREATE POLICY leave_roster_conflicts_tenant_isolation
ON leave_roster_conflicts
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'company_feed_posts_id_tenant_unique'
    ) THEN
        ALTER TABLE company_feed_posts
...
CREATE POLICY company_feed_drafts_tenant_isolation
ON company_feed_drafts
USING (
    tenant_id = NULLIF(current_setting('app.current_tenant', true), '')::UUID
)
WITH CHECK (
