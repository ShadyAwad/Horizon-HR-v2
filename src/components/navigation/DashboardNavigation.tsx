...
export function DashboardNavigation({ items, mobileShortcuts, onShortcutsChange, onOpenControlCenter, onLogout, userName, userEmail, lanyardSlot }: Props) {
  ...
  return <>
    <aside className={cn('fixed inset-x-3 bottom-[calc(.75rem+env(safe-area-inset-bottom))] z-40 flex items-center gap-1 rounded-2xl border border-emerald-500/15 bg-white/95 p-2 shadow-xl backdrop-blur-[6px] dark:bg-[#061411]/95 md:static md:inset-auto md:z-20 md:m-3 md:flex-col md:self-start md:p-2', isRtl ? 'md:order-last' : '')}>
      <button ref={launcherRef} id="stanza-control-center-trigger" type="button" aria-label={text('Open Stanza navigation', 'فتح تنقل Stanza')} aria-expanded={open} aria-controls="stanza-navigation-panel" onClick={() => setOpen((value) => !value)} className={cn('relative flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-emerald-500 text-[#020604] shadow-[0_0_18px_rgba(16,185,129,.24)] transition duration-200 ease-out hover:scale-105 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300', open && 'ring-2 ring-emerald-200')}><StanzaFingerprintMark size={24} /></button>
      <nav aria-label={text('Dashboard shortcuts', 'اختصارات لوحة التحكم')} className="hidden w-full flex-1 flex-col items-center gap-1 md:flex">{items.map((item) => itemButton(item, 'flex h-10 w-10 items-center justify-center rounded-lg border transition', () => choose(item)))}</nav>
      <button type="button" onClick={onOpenControlCenter} aria-label={text('Control Center', 'مركز التحكم')} className="hidden h-10 w-10 items-center justify-center rounded-lg text-slate-500 hover:bg-emerald-500/10 dark:text-emerald-100/65 md:flex"><Settings className="h-5 w-5" /></button>
    ...
    {open && <div id="stanza-navigation-panel" ref={panelRef} role="dialog" aria-modal="true" aria-label={text('Stanza navigation', 'تنقل Stanza')} className={cn('fixed z-30 w-[min(360px,calc(100vw-1.5rem))] overflow-hidden rounded-2xl border border-emerald-500/20 bg-white/98 shadow-2xl transition duration-200 ease-out dark:bg-[#061411]/98 md:start-[5.5rem] md:top-4', 'inset-x-3 bottom-[calc(5.5rem+env(safe-area-inset-bottom))] max-h-[76dvh] md:inset-x-auto md:bottom-auto md:max-h-[calc(100dvh-2rem)]')}>
      <div className="border-b border-emerald-500/15 p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-black text-slate-900 dark:text-emerald-50">Stanza</p><p className="mt-1 text-xs text-slate-500 dark:text-emerald-100/55">{userName}</p><p className="text-xs text-slate-500 dark:text-emerald-100/45">{userEmail}</p></div><button type="button" onClick={() => setOpen(false)} className="rounded-lg p-2 text-slate-500 hover:bg-emerald-500/10" aria-label={text('Close navigation', 'إغلاق التنقل')}><X className="h-4 w-4" /></button></div>{lanyardSlot && <div className="mt-3 h-16 overflow-hidden rounded-lg border border-emerald-500/15">{lanyardSlot}</div>}<label className="mt-3 flex items-center gap-2 rounded-lg border border-emerald-500/20 px-3 py-2 text-slate-500 focus-within:ring-2 focus-within:ring-emerald-400"><Search className="h-4 w-4" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={text('Search modules', 'البحث في الوحدات')} className="min-w-0 flex-1 bg-transparent text-sm text-slate-900 outline-none dark:text-emerald-50" /></label></div>
      <nav className="stanza-scrollbar max-h-[48dvh] overflow-y-auto p-2" aria-label={text('All authorised modules', 'كل الوحدات المصرح بها')}>{groups.map((group) => <section key={group} className="mb-3"><h2 className="px-2 pb-1 text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-emerald-100/45">{group}</h2>{visible.filter((item) => item.group === group).map((item) => <button key={item.id} type="button" onClick={() => choose(item)} aria-current={item.active ? 'page' : undefined} className={cn('flex min-h-11 w-full items-center gap-3 rounded-lg px-3 text-start text-sm font-bold transition', item.active ? 'bg-emerald-500/12 text-emerald-700 dark:text-emerald-200' : 'text-slate-700 hover:bg-emerald-500/10 dark:text-emerald-100/75')}><span className="relative">{item.icon}{item.badge ? <AttentionBadge count={item.badge} ariaLabel={`${item.label}: ${item.badge} action items`} className="absolute -end-2 -top-2" /> : null}</span><span className="flex-1">{item.label}</span></button>)}</section>)}</nav>
      <div className="flex gap-2 border-t border-emerald-500/15 p-3"><button type="button" onClick={() => { setOpen(false); onOpenControlCenter(); }} className="flex min-h-10 flex-1 items-center justify-center gap-2 rounded-lg border border-emerald-500/20 text-xs font-bold"><Settings className="h-4 w-4" />{text('Control Center', 'مركز التحكم')}</button><button type="button" onClick={onLogout} className="flex min-h-10 items-center justify-center gap-2 rounded-lg border border-red-500/25 px-3 text-xs font-bold text-red-700 dark:text-red-200"><LogOut className="h-4 w-4" />{text('Logout', 'تسجيل الخروج')}</button></div>
    </div>}
  </>;
}
