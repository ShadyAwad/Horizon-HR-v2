...
export function DashboardNavigation({ items, desktopMode = 'launcher', mobileShortcuts, onShortcutsChange, onOpenControlCenter, onOpenChange, showLanyardDock = false, onLogout, userName, userEmail, lanyardSlot }: Props) {
  ...
  return <>
    <aside className={cn(
      ...
      <button ref={launcherRef} id="stanza-control-center-trigger" type="button" aria-label={text('Open Stanza navigation', 'فتح تنقل Stanza')} title={text('Open Stanza navigation', 'فتح تنقل Stanza')} aria-expanded={open} aria-controls="stanza-navigation-panel" onClick={() => setOpen((value) => !value)} className={cn('relative flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-emerald-300/45 bg-emerald-500 text-[#020604] shadow-[0_0_18px_rgba(16,185,129,.24)] transition-[transform,box-shadow,background-color,border-color] duration-200 ease-[cubic-bezier(.2,.8,.2,1)] hover:-translate-y-px hover:scale-[1.03] hover:shadow-[0_0_22px_rgba(16,185,129,.30)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300 active:translate-y-0 active:scale-[.97] motion-reduce:transform-none motion-reduce:transition-none', open && 'scale-[1.03] border-emerald-100/80 bg-emerald-400 shadow-[0_0_24px_rgba(16,185,129,.34)]')}><StanzaFingerprintMark size={24} /></button>
