...
export function HiringPanel({ user, onRefreshAttentionCounts }: PanelProps) {
  ...
  const [applicantCreated, setApplicantCreated] = useState(false);
...
function ApplicantForm({
  ...
  submitLabel,
  reviewers,
  allowCandidateExtraction,
  creationCompleted,
}: {
  form: HiringApplicantInput;
  onChange: (value: HiringApplicantInput) => void;
  onSubmit: (event: React.FormEvent) => void;
  loading: boolean;
  submitLabel: string;
  reviewers: HiringReviewer[];
  allowCandidateExtraction: boolean;
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <CandidateDocumentExtraction
        enabled={allowCandidateExtraction}
        creationCompleted={creationCompleted}
        onApplySuggestion={updateCandidateField}
        onInitialSuggestions={applyInitialSuggestions}
        onProcessingChange={setExtractionProcessing}
      />
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        {field('fullName', t('hiring.fullName'))}
        {field('positionTitle', t('hiring.position'))}
        {field('email', t('hiring.email'), 'email')}
        {field('phone', t('hiring.phone'))}
        {field('department', t('hiring.department'))}
        {field('source', t('hiring.source'))}
        {field('appliedAt', t('hiring.applicationDate'), 'date')}
        {reviewers.length > 0 && <Select label={t('hiring.assignedReviewer')} value={form.currentOwnerId || ''} onChange={(currentOwnerId) => onChange({ ...form, currentOwnerId: currentOwnerId || undefined })} options={[['', t('hiring.unassigned')], ...reviewers.map((reviewer) => [reviewer.id, reviewer.displayName])]} />}
      </div>
      <Submit loading={loading || extractionProcessing} label={submitLabel} />
    </form>
  );
}
