...
export function TutorialOverlay({ tutorialId, steps, stepIndex, onBack, onNext, onSkip, onClose }: Props) {
  ...
  return <div className="fixed inset-0 z-[95]" data-tutorial-overlay={tutorialId} aria-live="polite">
    <div className="absolute inset-0 bg-black/45" aria-hidden="true" />
    {bounds && <div aria-hidden="true" className="pointer-events-none fixed rounded-xl border-2 border-emerald-300 shadow-[0_0_0_9999px_rgba(0,0,0,.36)] motion-reduce:transition-none" style={{ top: bounds.top - 4, left: bounds.left - 4, width: bounds.width + 8, height: bounds.height + 8 }} />}
    <section ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="tutorial-title" aria-describedby="tutorial-body" dir={isRtl ? 'rtl' : 'ltr'} className="fixed z-10 w-[min(20rem,calc(100vw-1.5rem))] rounded-xl border border-emerald-500/25 bg-white p-4 text-slate-900 shadow-2xl outline-none dark:bg-[#061411] dark:text-emerald-50 md:w-80" style={{ left, top }}>
